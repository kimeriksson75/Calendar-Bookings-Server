...
exports.getAll = (req, res, next) => bookingService.getAll()
...
exports.getByServiceDate = (req, res, next) => bookingService.getByServiceDate(req.params.service, req.params.date)
  .then(booking => res.status(200).json(booking))
  .catch(err => next(err));
  
exports.getByServiceMonth = (req, res, next) => bookingService.getByServiceMonth(req.params.service, req.params.date)
  .then(booking => res.status(200).json(booking))
  .catch(err => next(err));
  
exports.getByServiceUser = (req, res, next) => bookingService.getByServiceUser(req.params.service, req.params.id)
  .then(booking => res.status(200).json(booking))
  .catch(err => next(err));
