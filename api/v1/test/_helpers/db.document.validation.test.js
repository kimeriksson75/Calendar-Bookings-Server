const { existingDocumentById, isValidObjectId } = require('../../_helpers/db.document.validation');
...
describe('existingDocumentById()', () => {
    ...
    it('should throw an error if the document does not exists', async () => {
        ...
        await expect(existingDocumentById(mockDoc, validId)).rejects.toThrow(`Document with id ${validId} does not exists`);
    });

    it('should throw an error if the document id is not valid', async () => {
        const mockDoc = { findById: jest.fn().mockRejectedValue(new Error('invalid id')) };
        await expect(existingDocumentById(mockDoc, 'mock id')).rejects.toThrow('Invalid id mock id');
    });
});
...
describe('isValidObjectId()', () => {
    ...
    it('should throw an error if the id is not valid', () => {
        expect(() => isValidObjectId('invalid id')).toThrow('Invalid id invalid id');
    });
});