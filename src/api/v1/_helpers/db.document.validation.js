...
const existingDocumentById = async (doc, id) =>
  new Promise((resolve, reject) => {
    ...
    return doc
      ...
      .then((doc) =>
        doc
          ? resolve(doc)
          : reject(new NotFoundError(`Document with id ${id} does not exists`)),
      )
      .catch((err) =>
        reject(
          new ValidationError(
            err?.message || `Error while getting document with id ${id}`,
          ),
        ),
      );
  });
